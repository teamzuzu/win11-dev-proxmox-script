...
Windows 11 installs "new Outlook for Windows" via a Store-driven task that runs during OOBE itself, and that Store interaction can surface a region/market prompt that has nothing to do with the language/locale settings elsewhere in `autounattend.xml`. This is deprovisioned at first logon (see `CLAUDE.md`), so a freshly generated answer-file ISO shouldn't hit this — if you're still seeing it, rebuild the OEM ISO (rerun `win11.sh`) rather than reusing one generated before this fix.

### "The computer restarted unexpectedly" during Windows Setup
This is Windows Setup's generic fatal-error message for a `RunSynchronousCommand` returning a nonzero exit code in the `windowsPE`/`specialize` passes — it means something in `autounattend.xml` itself failed, not a hardware/VM problem. If you're seeing this on a build from before 2026-09-14, rebuild the OEM ISO (rerun `win11.sh`): an earlier version of this file ran the "new Outlook" removal from the `specialize` pass, which collided with Windows Setup's own concurrent servicing and crashed the install this way — it's since been moved to `FirstLogonCommands`, see `CLAUDE.md`. If it recurs after rebuilding, the specific broken command won't be obvious from this message alone; the next diagnostic step is pressing Shift+F10 at the failure screen for a command prompt and checking `X:\Windows\Panther\setupact.log`/`setuperr.log`.
