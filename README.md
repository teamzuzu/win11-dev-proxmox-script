...
Windows 11 installs "new Outlook for Windows" via a Store-driven task that runs during OOBE itself, and that Store interaction can surface a region/market prompt that has nothing to do with the language/locale settings elsewhere in `autounattend.xml`. **This is a known, unresolved cosmetic issue with no automated fix** — three separate attempts to remove/block Outlook (two before OOBE, one after) all either crashed Windows Setup or hung the first-logon script batch, all colliding with Windows' own concurrent Update Orchestrator activity around the same package (see `CLAUDE.md`). Outlook removal has been dropped from this script entirely as a result. Just click through the prompt if it appears — it doesn't indicate anything is actually broken. If you want Outlook gone afterward, remove it manually once the VM has been up and idle for a few minutes (i.e. well after first-logon churn has settled):
```powershell
...
### A PowerShell window sits open/unresponsive during first logon
A `FirstLogonCommands` entry has hung rather than completing - unlike a crash during Setup itself, this doesn't abort the install, it just blocks the rest of the (synchronous) batch until the window is closed. This happened once here (the "new Outlook" removal step, since removed - see `CLAUDE.md`) due to it colliding with a concurrent Windows background process. If you're on a build from before 2026-09-14, rebuild the OEM ISO. If a *different* command ever hangs here after that, closing the window lets the remaining steps continue (as they did in the one observed case) - the visible command text in the window itself is the best clue for which `FirstLogonCommands` entry it was, so it can be matched against `autounattend.xml` afterward.
